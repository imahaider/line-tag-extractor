...
# ---------------- Styling (title fix + green XLSX button)
st.markdown("""
<style>
...
.app-title {
  ...
  margin-bottom: .35rem; word-break: break-word; overflow-wrap: anywhere;
}
.footer {color: rgba(49,51,63,.55); font-size: .85rem; text-align:center; margin-top: 2rem;}
/* Green style for XLSX download (scoped) */
...
.dl-xlsx button:hover, .dl-xlsx [data-testid="baseButton-secondary"]:hover {
...
with st.sidebar:
    ...
    # Export first (so Regex can be placed below it)
    st.subheader("Export")
    export_fmt = st.segmented_control("Format", ["XLSX", "CSV", "TXT"], default="XLSX")

    st.markdown("---")
    # Regex Pattern now below Export
...
if uploaded_files and run:
    ...
    if all_tags:
        ...
        if export_fmt == "XLSX":
            ...
            st.download_button(
                "Download CSV", csv, "line_number_tags.csv", "text/csv", use_container_width=True
            )
        else:
            txt = "\n".join(df["Line Number Tags"].astype(str).tolist()).encode("utf-8")
            st.download_button(
                "Download TXT", txt, "line_number_tags.txt", "text/plain", use_container_width=True
            )
    else:
